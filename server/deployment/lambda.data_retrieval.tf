...
resource "null_resource" "build_data_retrieval" {
  triggers = {
    always_run = "${timestamp()}"
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../code/data_retrieval/build.sh"
  }
}
...
data "archive_file" "data_retrieval" {
  type        = "zip"
  output_path = "${path.module}/../code/data_retrieval.zip" # TODO: change here
  source_dir  = "${path.module}/../code/data_retrieval"     # TODO: change here

  depends_on = [
    null_resource.build_data_retrieval # TODO: change here
  ]
}

# Tells Terraform to create an AWS lambda function
# - Filename here corresponds to the output_path in archive_file.data_retrieval.
...
resource "aws_lambda_function" "data_retrieval" {
  ...
  function_name = "${var.group_name}_${terraform.workspace}_data_retrieval" # TODO: change here
  handler       = "handler.handler"
  runtime       = "python3.9" # TODO: change here
  timeout       = 60

  role             = aws_iam_role.iam_for_lambda.arn
  source_code_hash = data.archive_file.data_retrieval.output_base64sha256 # TODO: change here
...
resource "aws_lambda_permission" "data_retrieval" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.data_retrieval.function_name # TODO: change here
  principal     = "apigateway.amazonaws.com"

  source_arn = "${data.aws_apigatewayv2_api.api_gateway_global.execution_arn}/*/*/data_retrieval" # TODO: change here
}
...
resource "aws_apigatewayv2_integration" "data_retrieval" {
  api_id           = var.gateway_api_id
  integration_type = "AWS_PROXY"

  integration_uri    = aws_lambda_function.data_retrieval.invoke_arn # TODO: change here
...
resource "aws_apigatewayv2_route" "data_retrieval" {
  api_id    = var.gateway_api_id
  route_key = "GET /${var.group_name}/data_retrieval" # TODO: change here

  target = "integrations/${aws_apigatewayv2_integration.data_retrieval.id}" # TODO: change here
...
resource "aws_cloudwatch_log_group" "data_retrieval_log" {
  name              = "/aws/lambda/${aws_lambda_function.data_retrieval.function_name}" # TODO: change here
  retention_in_days = 7
  lifecycle {
