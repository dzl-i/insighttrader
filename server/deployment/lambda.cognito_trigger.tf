...
resource "null_resource" "build_cognito_trigger" {
  triggers = {
    always_run = "${timestamp()}"
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../code/cognito_trigger/build.sh"
  }
}
...
data "archive_file" "cognito_trigger" {
  type        = "zip"
  output_path = "${path.module}/../code/cognito_trigger.zip" # TODO: change here
  source_dir  = "${path.module}/../code/cognito_trigger"     # TODO: change here

  depends_on = [
    null_resource.build_cognito_trigger # TODO: change here
  ]
}

# Tells Terraform to create an AWS lambda function
# - Filename here corresponds to the output_path in archive_file.cognito_trigger.
...
resource "aws_lambda_function" "cognito_trigger" {
  ...
  function_name = "${var.group_name}_${terraform.workspace}_cognito_trigger" # TODO: change here
  handler       = "handler.handler"
  runtime       = "python3.9" # TODO: change here
  timeout       = 60

  role             = aws_iam_role.iam_for_lambda.arn
  source_code_hash = data.archive_file.cognito_trigger.output_base64sha256 # TODO: change here
...
resource "aws_lambda_permission" "cognito_trigger" {
  ...
  function_name = aws_lambda_function.cognito_trigger.function_name # TODO: change here
  principal     = "apigateway.amazonaws.com"

  source_arn = "${data.aws_apigatewayv2_api.api_gateway_global.execution_arn}/*/*/cognito_trigger" # TODO: change here
}
...
resource "aws_apigatewayv2_integration" "cognito_trigger" {
  api_id           = var.gateway_api_id
  integration_type = "AWS_PROXY"

  integration_uri    = aws_lambda_function.cognito_trigger.invoke_arn # TODO: change here
...
resource "aws_apigatewayv2_route" "cognito_trigger" {
  api_id    = var.gateway_api_id
  route_key = "GET /${var.group_name}/cognito_trigger" # TODO: change here

  target = "integrations/${aws_apigatewayv2_integration.cognito_trigger.id}" # TODO: change here
...
resource "aws_cloudwatch_log_group" "cognito_trigger_log" {
  name              = "/aws/lambda/${aws_lambda_function.cognito_trigger.function_name}" # TODO: change here
  retention_in_days = 7
  lifecycle {
