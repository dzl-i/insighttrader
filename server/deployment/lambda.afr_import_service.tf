...
resource "null_resource" "build_afr_import_service" {
  triggers = {
    always_run = "${timestamp()}"
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../code/afr_import_service/build.sh"
  }
}


# Tells Terraform to compress your source code with dependencies
data "archive_file" "afr_import_service" {
  type        = "zip"
  output_path = "${path.module}/../code/afr_import_service.zip" # TODO: change here
  source_dir  = "${path.module}/../code/afr_import_service"     # TODO: change here

  depends_on = [
    null_resource.build_afr_import_service # TODO: change here
  ]
}

# Tells Terraform to create an AWS lambda function
# - Filename here corresponds to the output_path in archive_file.afr_import_service.
...
resource "aws_lambda_function" "afr_import_service" {
  filename      = data.archive_file.afr_import_service.output_path
  function_name = "${var.group_name}_${terraform.workspace}_afr_import_service" # TODO: change here
  handler       = "handler.handler"
  runtime       = "python3.9" # TODO: change here
  timeout       = 60

  role             = aws_iam_role.iam_for_lambda.arn
  source_code_hash = data.archive_file.afr_import_service.output_base64sha256 # TODO: change here
...
resource "aws_lambda_permission" "afr_import_service" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.afr_import_service.function_name # TODO: change here
  principal     = "apigateway.amazonaws.com"

  source_arn = "${data.aws_apigatewayv2_api.api_gateway_global.execution_arn}/*/*/afr_import_service" # TODO: change here
}
...
resource "aws_apigatewayv2_integration" "afr_import_service" {
  api_id           = var.gateway_api_id
  integration_type = "AWS_PROXY"

  integration_uri    = aws_lambda_function.afr_import_service.invoke_arn # TODO: change here
...
resource "aws_apigatewayv2_route" "afr_import_service" {
  api_id    = var.gateway_api_id
  route_key = "GET /${var.group_name}/afr_import_service" # TODO: change here

  target = "integrations/${aws_apigatewayv2_integration.afr_import_service.id}" # TODO: change here
...
resource "aws_cloudwatch_log_group" "afr_import_service_log" {
  name              = "/aws/lambda/${aws_lambda_function.afr_import_service.function_name}" # TODO: change here
  retention_in_days = 7
  lifecycle {
