...
resource "null_resource" "build_afr_importer" {
  triggers = {
    always_run = "${timestamp()}"
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../code/afr_importer/build.sh"
  }
}


# Tells Terraform to compress your source code with dependencies
data "archive_file" "afr_importer" {
  type        = "zip"
  output_path = "${path.module}/../code/afr_importer.zip" # TODO: change here
  source_dir  = "${path.module}/../code/afr_importer"     # TODO: change here

  depends_on = [
    null_resource.build_afr_importer # TODO: change here
  ]
}

# Tells Terraform to create an AWS lambda function
# - Filename here corresponds to the output_path in archive_file.afr_importer.
...
resource "aws_lambda_function" "afr_importer" {
  filename      = data.archive_file.afr_importer.output_path
  function_name = "${var.group_name}_${terraform.workspace}_afr_importer" # TODO: change here
  handler       = "handler.handler"
  runtime       = "python3.9" # TODO: change here
  timeout       = 180

  role             = aws_iam_role.iam_for_lambda.arn
  source_code_hash = data.archive_file.afr_importer.output_base64sha256 # TODO: change here
  ...
  environment {
    variables = {
      ...
      DATA_IMPORTER_LAMBDA_NAME = aws_lambda_function.data_import.function_name
    }
  }
}
...
resource "aws_lambda_permission" "afr_importer" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.afr_importer.function_name # TODO: change here
  principal     = "apigateway.amazonaws.com"

  source_arn = "${data.aws_apigatewayv2_api.api_gateway_global.execution_arn}/*/*/afr_importer" # TODO: change here
}
...
resource "aws_apigatewayv2_integration" "afr_importer" {
  api_id           = var.gateway_api_id
  integration_type = "AWS_PROXY"

  integration_uri    = aws_lambda_function.afr_importer.invoke_arn # TODO: change here
...
resource "aws_apigatewayv2_route" "afr_importer" {
  api_id    = var.gateway_api_id
  route_key = "POST /${var.group_name}/afr_importer" # TODO: change here

  target = "integrations/${aws_apigatewayv2_integration.afr_importer.id}" # TODO: change here
...
resource "aws_cloudwatch_log_group" "afr_importer_log" {
  name              = "/aws/lambda/${aws_lambda_function.afr_importer.function_name}" # TODO: change here
  retention_in_days = 7
  lifecycle {
