...
resource "null_resource" "build_data_processing" {
  triggers = {
    always_run = "${timestamp()}"
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../code/data_processing/build.sh"
  }
}
...
data "archive_file" "data_processing" {
  type        = "zip"
  output_path = "${path.module}/../code/data_processing.zip" # TODO: change here
  source_dir  = "${path.module}/../code/data_processing"     # TODO: change here

  depends_on = [
    null_resource.build_data_processing # TODO: change here
  ]
}

# Tells Terraform to create an AWS lambda function
# - Filename here corresponds to the output_path in archive_file.data_processing.
...
resource "aws_lambda_function" "data_processing" {
  ...
  function_name = "${var.group_name}_${terraform.workspace}_data_processing" # TODO: change here
  handler       = "handler.handler"
  runtime       = "python3.9" # TODO: change here
  timeout       = 60

  role             = aws_iam_role.iam_for_lambda.arn
  source_code_hash = data.archive_file.data_processing.output_base64sha256 # TODO: change here
...
resource "aws_lambda_permission" "data_processing" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.data_processing.function_name # TODO: change here
  principal     = "apigateway.amazonaws.com"

  source_arn = "${data.aws_apigatewayv2_api.api_gateway_global.execution_arn}/*/*/data_processing" # TODO: change here
}
...
resource "aws_apigatewayv2_integration" "data_processing" {
  api_id           = var.gateway_api_id
  integration_type = "AWS_PROXY"

  integration_uri    = aws_lambda_function.data_processing.invoke_arn # TODO: change here
...
resource "aws_apigatewayv2_route" "data_processing" {
  api_id    = var.gateway_api_id
  route_key = "POST /${var.group_name}/data_processing" # TODO: change here

  target = "integrations/${aws_apigatewayv2_integration.data_processing.id}" # TODO: change here
...
resource "aws_cloudwatch_log_group" "data_processing_log" {
  name              = "/aws/lambda/${aws_lambda_function.data_processing.function_name}" # TODO: change here
  retention_in_days = 7
  lifecycle {
