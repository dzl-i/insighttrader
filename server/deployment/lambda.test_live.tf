...
resource "null_resource" "build_test_live" {
  triggers = {
    always_run = "${timestamp()}"
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../code/test_live/build.sh"
  }
}


# Tells Terraform to compress your source code with dependencies
data "archive_file" "test_live" {
  type        = "zip"
  output_path = "${path.module}/../code/test_live.zip" # TODO: change here
  source_dir  = "${path.module}/../code/test_live"     # TODO: change here

  depends_on = [
    null_resource.build_test_live # TODO: change here
  ]
}

# Tells Terraform to create an AWS lambda function
# - Filename here corresponds to the output_path in archive_file.test_live.
...
resource "aws_lambda_function" "test_live" {
  filename      = data.archive_file.test_live.output_path
  function_name = "${var.group_name}_${terraform.workspace}_test_live" # TODO: change here
  handler       = "handler.handler"
  runtime       = "python3.9" # TODO: change here
  timeout       = 60

  role             = aws_iam_role.iam_for_lambda.arn
  source_code_hash = data.archive_file.test_live.output_base64sha256 # TODO: change here
...
resource "aws_lambda_permission" "test_live" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.test_live.function_name # TODO: change here
  principal     = "apigateway.amazonaws.com"

  source_arn = "${data.aws_apigatewayv2_api.api_gateway_global.execution_arn}/*/*/test_live" # TODO: change here
}
...
resource "aws_apigatewayv2_integration" "test_live" {
  api_id           = var.gateway_api_id
  integration_type = "AWS_PROXY"

  integration_uri    = aws_lambda_function.test_live.invoke_arn # TODO: change here
...
resource "aws_apigatewayv2_route" "test_live" {
  api_id    = var.gateway_api_id
  route_key = "GET /${var.group_name}/test_live" # TODO: change here

  target = "integrations/${aws_apigatewayv2_integration.test_live.id}" # TODO: change here
...
resource "aws_cloudwatch_log_group" "test_live_log" {
  name              = "/aws/lambda/${aws_lambda_function.test_live.function_name}" # TODO: change here
  retention_in_days = 7
  lifecycle {
