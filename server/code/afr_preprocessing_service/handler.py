...
def pub_metric(metric, value, unit):
    cloudwatch.put_metric_data(
        Namespace='SE3011-24-F11A-02-PreprocessingService',
        MetricData=[
            {
                'MetricName': metric,
                'Value': value,
...
def df_to_adage(df, s3):
    ...
    for _, row in df.iterrows():
        try:
            ...
            logger.error(f"Preprocessing Service - Error processing row: {str(e)}")
...
def handler(event, context):
    ...
    successful_file_processing_count = 0
    
    segment = xray_recorder.begin_segment('handler')
    
    try:
        logger.info("Handler invoked - Preprocessing Service")
        s3 = boto3.client(
            "s3",
            aws_access_key_id="AKIAVRUVQGYFWWABVYBW",
            aws_secret_access_key="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"
        ...
        try:
            with xray_recorder.in_subsegment('s3_data_retreival') as seg:
                dataset_id = body["dataset_id"]
                start_year = int(body.get("start_year", "0"))
                end_year = int(body.get("end_year", "9999"))

                path = f"SE3011-24-F11A-02/processed_data/{dataset_id}"
                jsons = s3.list_objects_v2(Bucket="seng3011-student", Prefix=path).get("Contents", [])
                if len(jsons) == 0:
                  return {
                      "statusCode": 404,
                      "body": json.dumps({"status": "Error", "message": "Dataset not found"}),
                      "headers": {
        ...
        logger.info(f"Preprocessing Service - Processing files: {jsons}")
        
        logger.info(f"Preprocessing Service - Start year: {start_year}")
        logger.info(f"Preprocessing Service - End year: {end_year}")
        try:
            with xray_recorder.in_subsegment('data_preprocessing') as seg:
                for file in jsons:
                    file_name = file['Key']
                    logger.info(f"Preprocessing Service - Processing file: {file_name}")
                    year = file_name.split("/")[-1].split(".")[0][:4]
                    if not year or int(year) < start_year or int(year) > end_year:
                        continue
                    
                    adage_data_model = s3.get_object(Bucket="seng3011-student", Key=file['Key'])['Body'].read().decode('utf-8')
                    logger.info(f"Preprocessing Service - Loaded data from S3, Size: {len(adage_data_model)} Bytes")
                    
                    df = adage_to_df(adage_data_model)
                    logger.info(f"Preprocessing Service - Converted S3 data to DataFrame,  Rows: {len(df)}")
                    ...
                    df['pre_processed_text'] = df['text'].apply(lambda x: clean_and_prepare_text(x, pattern, lower, stopword_removal, stem))
                    ...
                    preprocessing_duration = end_preprocess - time_preprocess
                    logger.info(f"Preprocessing Service - Preprocess Duration for file {file_name}: {preprocessing_duration:.2f} seconds")
                    pub_metric('PreprocessingDurationPerFile', preprocessing_duration, 'Seconds')
                    adage_data_model = df_to_adage(df, s3)
                    
                    s3.put_object(Bucket="seng3011-student", Key=f"SE3011-24-F11A-02/test_preprocessed/afr/{year}.json", Body=adage_data_model)
                    logger.info(f"Preprocessing Service - Processed file {file_name}")
                    successful_file_processing_count += 1
        except Exception as e: 
            raise
        ...
        logger.info(f"Preprocessing Service - Total Execution time: {exec_time:.2f} seconds")
        total_mem = psutil.Process().memory_info().rss/1024/1024
        pub_metric('MemoryUsage', total_mem, 'Megabytes')
        logger.info(f"Preprocessing Service - Memory usage: {total_mem:.2f} MB")
        
        pub_metric('SuccessfulFileProcessingCount', successful_file_processing_count, 'Count')
        ...
        logger.error(f"Preprocessing Service - Error occured while preprocessing data: {str(e)}")
        pub_event_log(msg)
        return {
            "statusCode": 500,
