...
def handler(event, context):
    ...
    try:
        syd_time = pytz.timezone('Australia/Sydney')
        body = event.get('Records', [])
        for record in body:
            current_syd_time = datetime.now(syd_time)
            
            if isinstance(record['body'], str):
                record_data = json.loads(record['body'])
            elif isinstance(record['body'], dict):
                record_data = record['body']
    
            record = record_data
            record['modified'] = current_syd_time.isoformat()
            ...
            df[['sentiment_polarity', 'sentiment_subjectivity']] = df['text'].apply(lambda x: pd.Series(sentiment(x)))
            ...
            cursor.execute("""
                UPDATE articles SET
                    ...
                    topic = %s
                WHERE guid = %s;
            """, (article['sentiment_polarity'], article['sentiment_subjectivity'], article['avg_sentence_length'], article['topic'], article['guid']))
            
            if 'named_entities' in article:
                unique_entities = list(set(article['named_entities'][:14]))
                ...
                article_entities = [(article['guid'], entity_to_id[entity]) for entity in article['named_entities'][:14] if entity in entity_to_id]
                execute_values(cursor,
